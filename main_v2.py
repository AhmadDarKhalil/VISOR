...
def get_seg_info(args):
    if not os.path.exists(args.m):
        map_info = find_map('./selected_frames_for_Toronto/ori', './selected_frames_for_Toronto/pro')
    else:
    ...
    segs = []
    for i, each in enumerate(annotation_info):
        if 'annotation' not in each:
            continue
        action = each["documents"][0]["directory"]
        vid, fid = get_frame_info(each, map_info)
        if vid is None or '.' in vid:
            continue
        masks = get_masks_info(each)
        if any(masks['masks']) and args.vis:

#            fid = 'frame_0000005741.jpg'
#            vid = 'P30_107'
#            action = 'P30_107-rinse_tea_strainer_003'
#            with open('a.json', 'r') as f:
#                mi = json.load(f)
#            masks = get_masks_info(mi[0])

            vis_masks(os.path.join(args.img, action, "_".join([vid,fid])), masks)
            #vis_masks_ahmad(masks)
        d = {"vid": vid, "fid": fid, "masks": masks}
        segs.append(d)
        generate_clip(args, vid, fid)
    return segs
...
def generate_images_n_V2(video_path,dest_folder,start_frame_id,number_of_frames):
    ...
    x = "ffmpeg -loglevel error -threads 16 -ss "+str(start_idx)+" -i "+video_path+" -qscale:v 4 -qscale 2 -vf scale=854:480 -frames:v  "+str(number_of_frames)+" "+dest_folder+"/"+prefix+"frame__%10d.jpg"
    print(x)
    os.system(x)
def generate_images_n_V2_index1(video_path,dest_folder,start_frame_id,number_of_frames):
    ...
    start_idx = start_idx/frame_rate
    x = "ffmpeg -loglevel error -ss "+str(start_idx)+" -i "+video_path+" -qscale:v 2 -vf scale=854:480 -frames:v  "+str(number_of_frames)+" "+dest_folder+'/'+prefix+"frame__%10d.jpg"
    print(x)
    os.system(x)
...
def visualize_stats_v2():
    import numpy as np
    from matplotlib import pyplot as plt
    n2 = np.load("N_2.npy")
    n3 = np.load("N_3.npy")
    fig, ax = plt.subplots()
    t = range(0,n2.shape[0])
    ax.plot(t, np.sort(n2[:,0]), 'r.',label='Number of actions=2')
    ax.plot( t, np.sort(n3[:,0]), 'b.',label='Number of actions=3')
    leg = ax.legend()
    ax.legend(loc='upper left', frameon=False)
    plt.xlabel('Video number (sorted on duration)')
    plt.ylabel('Duration (secodns)')
    plt.ylim([0, 70])
    plt.show()
...
def do_stats_stage2_jsons(folder):
    ...
    for infile in sorted(glob.glob(folder + '/*.json')):
        ...
        total_repos = total_repos + 1
        #sort based on the folder name (to guarantee to start from its first frame of each sequence)
        data = sorted(data, key=lambda k: k['image']['image_path'])
...
def do_stats_stage2_jsons_single_file(file):
    ...
    for datapoint in data:
        obj_per_image=0 # count number of objects per image
        seq = datapoint['documents'][0]['subsequence']
        if (seq != prev_seq):
            total_number_of_seq = total_number_of_seq + 1
            prev_seq = seq
        image_name = datapoint["documents"][0]["name"]
...
def split_seq_into_2_masks_pairs_test(folder): # this to generate dense images for seq (e.g. start and end frame of the seq)
    import os
    import glob
    import cv2
    import numpy as np
    ...
    for subdir, dirs, files in sorted(os.walk(folder)):
        index = 0
        kk = 3 # how many images to perform a seq
        if (subdir != folder):
            path480 = subdir.replace("/masks/", "/masks/")
            print(path480)
            i=0
            seq_files = sorted(glob.glob(path480 + '/*.png'))
            for i in range(0,len(seq_files)-kk-1,kk):
                pairs_folder = os.path.join(os.path.join("/".join(path480.split("/")[:-2]),"pairs_masks"),seq_files[i].split("/")[-2]+"_"+'{:04d}'.format(i))
                os.makedirs(pairs_folder, exist_ok=True, mode=0o777)
                # Source path
                for k in range(0,kk+1):
                    ...
                    shutil.copyfile(source, destination)
                start_frame = seq_files[i].split("/")[-1].split(".")[0]
                end_frame = seq_files[i+k].split("/")[-1].split(".")[0]
                print(f"start: {start_frame} and end: {end_frame}")
                image_path480 = path480.replace("/masks/", "/images_48050fps/")
                seq_image_files = sorted(glob.glob(image_path480 + '/*.jpg'))
...
def generate_dense_images_from_masks(input_dir,output_dir):
    ...
    for infile in sorted(glob.glob(input_dir + '*/*.png')):
        print (infile)
        v = '_'.join(infile.split("/")[-1].split('_')[0:2])
        print('v',v)
        video_path = '../videos/' + v+'.MP4'
        if seq_name != infile.split("/")[-2]: #check if the current seq is not the same with previous one (new seq)
           if ((seq_name != "")  and (not os.path.exists(os.path.join(output_dir,seq_name)))): # if it is not the before starting (seq = "" as inital value)
                #print(len(os.walk("/".join(infile.split("/")[:-2])).__next__()[1]))
                #os.makedirs(os.path.join(output_dir,seq_name+"_"+str(seq_start_frame)+"_"+str(seq_end_frame)), exist_ok=True) this for start and end frame
                os.makedirs(os.path.join(output_dir,seq_name), exist_ok=True)
                print("Seq: ",seq_name)
                ...
                print("END: ",seq_end_frame)
                print("-----------------------")
                current_v = '_'.join(seq_start_frame_str.split('_')[0:2])
                #import pdb
                #pdb.set_trace() 
                generate_images_n_V2(video_path.replace(v,current_v), os.path.join(output_dir,seq_name), seq_start_frame_str,
                                     seq_end_frame - seq_start_frame + 1)
                rename_images(os.path.join(output_dir, seq_name), seq_start_frame_str,seq_start_frame,seq_end_frame - seq_start_frame + 1)
                copy_jpg_image(seq_start_frame_str,"./Images_fixed",os.path.join(output_dir,seq_name))
                copy_jpg_image(seq_end_frame_str,"./Images_fixed",os.path.join(output_dir,seq_name))
                #pdb.set_trace()
...
def keep_similar_objects(folder):
    ...
    t_total = 0
    df = pd.read_csv(batch+'_data_mapping.csv')
    ...
    for filename in sorted(os.listdir(folder)):
        ...
        if os.path.isdir(f):
            ...
            vid = '_'.join(images[0].split('/')[-1].split('_')[:2])
            ...
            if common == 0: #then delete the interpolation as no shared masks
                ...
                with open(batch+"_repos.txt", "a") as myfile:
...
'''
    ...
    if objects_keys:
        ...
        data['video_id'] = '_'.join(image1_path.split('/')[-1].split('_')[:2])
        data['interpolation'] = image1_path.split('/')[-2]
        data['Object_name'] = objs
        if not os.path.isfile(batch+'_int_data_mapping.csv'):
            data.to_csv(batch+'_int_data_mapping.csv', index=False,header=['old_index', 'new_index','video_id','interpolation','Object_name'])
        else:
            data.to_csv(batch+'_int_data_mapping.csv',mode='a', header=False,index=False)
...
def delete_less_than_n_v2(min_frames,max_frames,masks_path):
    import glob
    import os
    import shutil
    min=100000
    ...
    count=0
    fcount=0
    more_than_200_count = 0
    #batch_02/masks/*
    #/mnt/storage/scratch/ru20956/DAVIS_FOR_EVAL/Annotations/480p/*
    #/mnt/storage/scratch/ru20956/DAVIS_FOR_EVAL/JPEGImages/480p/*

    for folder in sorted(glob.glob(masks_path+'/*')):
        #print(folder)     
        f=0
        file1 = sorted(glob.glob(folder+"/*.png"))[0]
        file2 = sorted(glob.glob(folder+"/*.png"))[1]
        file_num1 = int(file1.split('/')[-1][-14:-4])
        file_num2 = int(file2.split('/')[-1][-14:-4])

        diff  = file_num2 - file_num1
        #print(str(file_num), ', prev:', str(prev_file_num), ' diff: ', diff)
        if diff <= min_frames:
            min = diff + 1
            #print(file, ' min:', diff)
        if diff > max:
            max = diff + 1
            #print(file, ' max:', max)
        if diff > max_frames or diff <= min_frames:
            more_than_200_count = more_than_200_count + diff + 1
            shutil.rmtree(folder)
            #print(folder, ' repos:',  diff + 1)

            f=1
        count = count + diff + 1
        fcount = fcount +1
#        if f == 0 and os.path.exists(folder.replace('Annotations','JPEGImages')):
#            batch = 'batch_06'
#            with open(batch+"_repos_f.txt", "a") as myfile:
#                myfile.write(folder.split('/')[-1])
#                myfile.write('\n')
#                myfile.close()
#        if f == 1:
#            shutil.rmtree(folder.replace('Annotations','JPEGImages'))
#            shutil.rmtree(folder)

    print('Minimum number of frame difference is: ', min)
    print('Max number of frame difference is: ', max)
    print('Number of images: ', count , 'repos count: ', fcount)
    print(f'Number of images more than {max_frames} repos: ', more_than_200_count)
...
def move_folders(src, dest):
    ...
    for folder in os.listdir(src):
        full_path_scr = os.path.join(src,folder)
        full_path_dest = os.path.join(dest,folder)
        
        if os.path.isdir(full_path_scr) and not os.path.exists(full_path_dest):
            print('move: ',folder , ' to ', dest)
            shutil.copytree(full_path_scr, full_path_dest)



            
def main_f():
    #actions_to_seq_csv('EPIC_100_train_new.csv','EPIC_100_train_new_seq.csv','stage1_check_frames_copy')
    #actions_histogram('EPIC_100_train_new.csv','stage1_check_frames_copy') # it will do check for all actions in csv file and filter to the json folder
    #delete_overlap_csv_all_videos("filtered_EPIC_100_train.csv")
    #filter_acions_to_existing_videos('count_all_Jian.csv','stage1_check_frames_copy')
    #overlap_only('EPIC_100_train.csv','EPIC_100_train_new.csv')

    #out_of_actions_frames('EPIC_100_train.csv','stage1_check_frames_copy')
    #check_frame_rate('completed_annotations_stage1') # completed_annotations_stage1 stage1_60fps_data
    #do_stats_stage1_jsons('completed_annotations_stage1')
    #do_stats_stage2_jsons('completed_annotations_stage2')

    #fill_objects_gaps_f ('0011_P02_124_Part_001_fixed.json')
    #delete_overlap_csv_all_videos('EPIC_100_train_final.csv')
    ##filter_annotators('P07_110_all_amt_output.json',5,4,'P07_110_all_amt_output_updated.json')

    #get the dataset from the json files from stage 1
    #generate_N_sequence_of_actions_non_uniform('/Users/ahmaddarkhalil/Downloads/P07_110.mp4','0018','P07_110_all_amt_output_updated.json','P07_110','EPIC_100_train_new.csv',100,'P07_110/',3)

    ##fill_objects_gaps('0008_P02_121_Part_001.json')
    #to generate from toronto website to images and masks
    #combine_jsons_torento('jsons')
    #json_to_masks_vos('combined_annotations.json','masks_vos_3')
    #resize_all_images("masks_vos_3/images50fps/")
    #resize_all_masks("masks")

    #generate_50fps_from_2fps('/Users/ahmaddarkhalil/Downloads/P03_120.mp4','masks_vos_3/images/','masks_vos_3/images50fps/')
    #resize_all_images("masks_vos_2/images50fps/")

    ##for one json file
    #json_to_masks('sdrjan_modified.json','masks_P02_109_srdjan_modified/')
    #generate_flat_data_png('overlayed_masks_P02_109_srdjan_modified')

    ##for folder contains json files
    #folder_of_jsons_to_masks('completed_annotations_stage2_subset','segmentations_subset/')
    #generate_videos_from_all_masks('overlayed_segmentations_subset','overlayed_segmentations_videos_subset',3)

    #generate_videos_from_all_masks_vis('n',3)
    #copy_jpg_images('n')


    #viz_seg('t37246.json')

    #seq_to_video('masks_vos_2/masks_480/P30_107_seq_00058',6)
    #seq_to_video('masks_vos_2/masks_480','video_seq',6)
    #all_seq_to_videos_stage1('/Users/ahmaddarkhalil/Downloads/P01_104.mp4','P01_104','video_seq_n',50)
    #seq_to_videos_stage1('/Users/ahmaddarkhalil/Downloads/P01_104.mp4','P01_104/0001_P01_104_Part_001/0001_P01_104_seq_00010','video_seq_n',50,["Car","Hob/ storeve","TEST ME","sfsfsf","Hallo"])
    #generate_flat_data_part_by_part_5d('P01_01')
    #generate_flat_data_all_video('P04_113_new')
    #seq_to_video_jpg('P04_113_new_flat/P04_113_new/P04_113_Part_003', 'video_seq_test', 6)
    #all_seq_to_videos('masks_2','video_seq',6)
    #all_seq_to_videos('STM_output', 'video_seq_stm',50)
    #all_seq_to_videos('masks_3', 'video_seq_test', 3)
    #all_seq_to_videos('/Users/ahmaddarkhalil/Downloads/r2/results', 'video_seq_test', 3)

    #resize_images('./masks_vos/P30_107/Part_008/P30_107_seq_00074_jpg')
    #resize_masks('./masks_vos/P30_107/Part_008/P30_107_seq_00074')

    #generate_flat_data('masks_P02_109_robert')
    #generate_flat_data_png('masks_P02_109_joseph')


    #measure_blur('P04_113_flat/P04_113/')
    ...
    #delete_less_than_n_v2(5,300,batch+'/pairs_masks')
    #keep_similar_objects(batch+'/pairs_masks')
    #generate_dense_images_from_masks(batch+'/pairs_masks/',  '/jmain02/home/J2AD001/wwp01/shared/data/DAVIS_FOR_INTER/JPEGImages/480p/')
    #move_folders(batch+'/images/', '/jmain02/home/J2AD001/wwp01/shared/data/DAVIS_FOR_INTER/JPEGImages/480p')
    #move_folders(batch+'/pairs_masks/', '/jmain02/home/J2AD001/wwp01/shared/data/DAVIS_FOR_INTER/Annotations/480p')

    
    #resize_all_images("sample3/images50fps/")
    #resize_all_masks('sample3/masks/')



    #generate_videos_from_all_masks_STM('/Users/ahmaddarkhalil/Downloads/r2/results_one_pass_backward','video_seq_test',25)
    #generate_videos_from_all_masks_STM('overlayed_segmentations_subset', 'overlayed_segmentations_videos_subset', 3)
    ...
    #visualize_stats_v2()
    #filter_hands_objects('test.json')

    #json_to_csv_stage1('P04_113_1807.json','P04_113_1807.csv')
    #csv_to_json_stage1( 'P04_113_1807_edited.csv','P04_113_1807_edited.json')


    #generate_N_sequence_of_actions('/Users/ahmaddarkhalil/Downloads/P30_107.mp4','000','P30_107_0207.json','P30_107','EPIC_100_train_final.csv',2,100,'P30_107/',3)

    #generate_50fps_from_2fps('/Users/ahmaddarkhalil/Downloads/P30_112.mp4','sample/images/','sample/images50fps/')

    #generate_sequence_of_actions('/Users/ahmaddarkhalil/Downloads/P30_107.mp4','P30_107','EPIC_100_train_final.csv',3)
    #generate_sequence_of_actions_all_dataset('/Users/ahmaddarkhalil/Downloads/','EPIC_100_train_final.csv',3)
    #generate_images('./numbers/out.mp4','./numbers/')
    #generate_images_n('/Users/ahmaddarkhalil/Downloads/P30_107.mp4', './P30_107_n/','frame_0000000002',1)
    #generate_images_n_V2('/Users/ahmaddarkhalil/Downloads/P30_107.mp4', './P30_107_n/','frame_0000000001.jpg',3)
    #generate_epic_train_dataset_info('EPIC_100_train.csv','P30_107','frame_00000147000.jpg')
    #annotations_to_datatset('sample_actions.json',"./actions_traning_data")
    #args = parse_args()
    #segs = get_seg_info(args)
    #a=1

    #rename_and_resample_masks('STM_output_50fps','png')
